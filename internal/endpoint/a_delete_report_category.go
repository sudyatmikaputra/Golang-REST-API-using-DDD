...
func DeleteReportCategoryForAdmin(application feedbackReportApp.Application) endpoint.Endpoint {
	return func(ctx context.Context, req interface{}) (res interface{}, err error) {
		payload := req.(*public.DeleteReportCategoryRequest)
		...
		err = database.RunInTransaction(ctx, config.DB(), func(ctx context.Context) (err error) {
			...
			err = application.Commands.DeleteReportCategory.Execute(ctx, *payload)
