...
func UpdateFeedbackParameterForAdmin(application feedbackReportApp.Application) endpoint.Endpoint {
	return func(ctx context.Context, req interface{}) (res interface{}, err error) {
		payload := req.(*public.UpdateFeedbackParameterRequest)
		...
		err = database.RunInTransaction(ctx, config.DB(), func(ctx context.Context) (err error) {
			...
			res, err = application.Commands.UpdateFeedbackParameterForAdmin.Execute(ctx, *payload)
