...
func UpdateReportParameterForAdmin(application feedbackReportApp.Application) endpoint.Endpoint {
	return func(ctx context.Context, req interface{}) (res interface{}, err error) {
		payload := req.(*public.UpdateReportParameterRequest)
		...
		err = database.RunInTransaction(ctx, config.DB(), func(ctx context.Context) (err error) {
			...
			res, err = application.Commands.UpdateReportParameterForAdmin.Execute(ctx, *payload)
