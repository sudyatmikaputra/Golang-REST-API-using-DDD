...
func CreateReportForPatientDoctorToMedicplus(application feedbackReportApp.Application) endpoint.Endpoint {
	return func(ctx context.Context, req interface{}) (res interface{}, err error) {
		payload := req.(*public.CreateReportRequest)
		...
		err = database.RunInTransaction(ctx, config.DB(), func(ctx context.Context) (err error) {
			...
			res, err = application.Commands.CreateReportForPatientDoctorToMedicplus.Execute(ctx, *payload)
