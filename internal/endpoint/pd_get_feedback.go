...
func GetFeedbackForPatientDoctor(application feedbackReportApp.Application) endpoint.Endpoint {
	return func(ctx context.Context, req interface{}) (res interface{}, err error) {
		payload := req.(*public.GetFeedbackRequest)
		...
		err = database.RunInTransaction(ctx, config.DB(), func(ctx context.Context) (err error) {
			...
			res, err = application.Queries.GetFeedbackForPatientDoctor.Execute(ctx, *payload)
