...
func UpdateReportForPatientDoctor(application feedbackReportApp.Application) endpoint.Endpoint {
	return func(ctx context.Context, req interface{}) (res interface{}, err error) {
		payload := req.(*public.UpdateReportRequest)
		...
		err = database.RunInTransaction(ctx, config.DB(), func(ctx context.Context) (err error) {
			...
			res, err = application.Commands.UpdateReportForPatientDoctor.Execute(ctx, *payload)
